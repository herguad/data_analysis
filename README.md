...
Among `seaborn.scatterplot` arguments, both `size` and `hue` were set to represent the number of movies released each year per genre but the `legend` was set to False to avoid cluttering the overview of the data.
- Which genres got the most movies released in different years?
- Which were the top 3 genres for movies released in different years?
